...
Jev (TypeSafe System One) answers typed questions in parallel and returns calibrated
probabilities. It never writes prose. Control flow stays in YOUR code; this module
...
def ask(state, questions: dict, *, model: str | None = None, url: str | None = None) -> dict:
    ...
    if not key:
        raise RuntimeError("no OPENROUTER_API_KEY / TYPESAFE_API_KEY (see ~/.config/siso/.env)")
    ...
    req = urllib.request.Request(
        url or DEFAULT_URL, data=json.dumps(body, ensure_ascii=False).encode(), method="POST",
