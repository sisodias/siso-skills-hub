import { readFile, readdir, lstat, mkdir, writeFile, appendFile, mkdtemp, access } from 'node:fs/promises';
...
const inside = relative(input, handoff);
if (input === handoff || (inputStat.isDirectory() && inside !== '..' && !inside.startsWith('../') && !inside.startsWith('/'))) throw new Error('Handoff must be outside public input');
await access(dirname(handoff));
try { if ((await lstat(handoff)).isSymbolicLink()) throw new Error('Handoff symlinks are refused'); } catch (error) { if (error.code !== 'ENOENT') throw error; }
...
console.log(JSON.stringify({ verdict: 'preflight-pass', files: files.length, artifact_sha256: artifact }));
if (!options['--dry-run']) {
